    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader className="pb-0">
        <div className="flex justify-between items-center mb-4">
          <CardTitle className="text-lg md:text-xl">Task Templates</CardTitle>
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-3">
          <div className="flex-1">
            <div className="relative">
              ...
              <svg 
                xmlns="http://www.w3.org/2000/svg" 
                className="h-3.5 w-3.5 absolute top-[11px] right-3 text-gray-400" 
                viewBox="0 0 24 24" 
                fill="none" 
                stroke="currentColor" 
                strokeWidth="2" 
          ...
          <div className="flex h-9 items-center">
            <Button 
              variant="ghost" 
              size="sm" 
              className={`px-1.5 py-1 rounded-l-md ${viewMode === 'grid' ? 'bg-gray-100' : ''}`}
            ...
            <Button 
              variant="ghost" 
              size="sm" 
              className={`px-1.5 py-1 rounded-r-md ${viewMode === 'list' ? 'bg-gray-100' : ''}`}
      ...
      <CardContent className="p-4 pt-6">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <div className="overflow-x-auto pb-2 -mx-4 px-4" style={{WebkitOverflowScrolling: 'touch'}}>
            <TabsList className="inline-flex w-full whitespace-nowrap mb-3" style={{touchAction: 'pan-x'}}>
              {taskTemplateData.map((category) => (
                <TabsTrigger 
                  key={category.value} 
                  value={category.value}
                  className="flex items-center justify-center gap-1 flex-1 min-h-[36px] px-1"
                  style={{WebkitTapHighlightColor: 'transparent', touchAction: 'manipulation'}}
                >
                  {category.icon}
                  <span className="text-[10px] md:text-xs whitespace-nowrap hidden sm:inline">{category.label}</span>
                  <span className="text-[10px] sm:hidden whitespace-nowrap">{category.label.split('/')[0].trim()}</span>
                </TabsTrigger>
              ))}
            </TabsList>
          </div>
          ...
          {taskTemplateData.map((category) => {
            ...
            return (
              <TabsContent key={category.value} value={category.value} className="max-h-[55vh] overflow-y-auto pb-2" style={{WebkitOverflowScrolling: 'touch'}}>
